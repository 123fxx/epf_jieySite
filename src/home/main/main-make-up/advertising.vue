<template>
  ...
  >
    <div class="advertising-child" v-if="list.length > 0">
      <p v-for="(item, index) in list" :key="index" style="cursor: pointer">
        <span @click="toDetail(item)"> {{ index + 1 }}{{ item.name }}</span>
      </p>
    </div>
    <div class="close" @click="close" v-if="list.length > 0"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$get("/ords/epfcms/cmsItem/queryCmsItemFloat", {}).then((res) => {
        this.list = res.items;
      });
    },
    over() {
