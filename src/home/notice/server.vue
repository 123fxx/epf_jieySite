    ...
    <div class="tab-contain">
      <div class="tab-warp display">
        ...
        >
          ...
          <span class="line" v-if="item.no===active"></span>
        </div>
      </div>
    </div>
    <el-table
    ...
    >
      <template slot="empty">
        <div class="empty-contain">
          <img src="@/assets/image/home/noData.png" alt class="empty-img" />
          <div>暂无数据</div>
        </div>
      </template>
      <el-table-column
      ...
      <el-table-column label="文件名称" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <span @click="toDetail(scope.row)" class="table-title">{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="info_time2" label="日期" align="center" :show-overflow-tooltip="true"></el-table-column>
    </el-table>
  ...
  methods: {
    ...
    init(eng) {
      this.editableTabs = JSON.parse(
        window.sessionStorage.getItem(eng)
      ).children;
